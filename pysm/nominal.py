...
data_dir = os.path.join(os.path.dirname(__file__), 'lb_template')
lb_template = lambda x: os.path.join(data_dir, x)
...
def LB_d0(nside, pixel_indices=None, mpi_comm=None):
    ...
    return [{
        ...
        'nu_0_P': 353.,
        'A_I': A_I,
        'A_Q': read_map(template('dust_Q_ns512.fits'), nside, field=0, pixel_indices=pixel_indices, mpi_comm=mpi_comm),
        'A_U': read_map(template('dust_U_ns512.fits'), nside, field=0, pixel_indices=pixel_indices, mpi_comm=mpi_comm),
        'spectral_index': np.ones(len(A_I)) * 1.53,
        'temp': np.ones(len(A_I)) * 19.6,
        'add_decorrelation': False,
    }]

def LB_s0(nside, pixel_indices=None, mpi_comm=None):
    ...
    return [{
        ...
        'nu_0_P': 23.,
        'A_I': A_I,
        'A_Q': read_map(template('synch_Q_ns512.fits'), nside, field=0, pixel_indices=pixel_indices, mpi_comm=mpi_comm),
        'A_U': read_map(template('synch_U_ns512.fits'), nside, field=0, pixel_indices=pixel_indices, mpi_comm=mpi_comm),
